...
public class Fibonacci {
	...
	public String multiplicacion(String x, String d) {
		...
		parla.append(
				"Generamos una matriz del tama�o en filas y columnas equivalente a la cantidad de cifras de las dos entradas\n");
		byte matriz[][] = new byte[x.length()][d.length()];
		parla.append("tama�o de la nueva matriz: " + x.length() + " filas * " + d.length() + " columnas\n");
		parla.append(
				"Luego determinamos el tama�o m�ximo de cifras que puede tener la respuesta dada por la suma de las cifras de la entrada\n");
		int tamanio = x.length() + d.length();
		byte[] numerosResultado = new byte[tamanio];
		parla.append("cantidad de cifras m�xima de la salida: " + tamanio + "\n");
		...
		parla.append("Se aplica la regla de signos b�sica y almacenamos su carga, la cual es ");
		if (negativo)
			parla.append("negativa para este caso\n");
		else
			parla.append("positiva para este caso\n");
		parla.append("Se llena la matriz con las multiplicaciones de cada d�gito de los n�meros, as�:\n   ");
		for (; i < d.length(); i++) {
		...
		parla.append("\nDonde (#) es el d�gito proporcionado por la suma de su diagonal respectiva\n");
		parla.append("pasadas sus decenas a la suma del d�gito anterior tenemos\n");
